...
inventor.filter(invetor => inventor.year>=1500 && inventor.year < 1600)
}
...
export function map() {
inventors.map(curElem => `${curElem.first} ${curElem.last}`)
}
...
return inventors.reduce((accumulators, curElem) => accumulator += (curElem.passed - curElem.year))
}
